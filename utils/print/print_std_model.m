...
%  LastEditTime: 2024-02-24 16:13:25
%  
...
%   
function print_std_model(model, fid)
    ...
    for p = 1: length(At)
        ...
        fprintf(fid, "\tblock %i: %i x %i, norm  = %.4e, nnz = %.4e, sparsity = %.4e\n", ...
            p, size(At{p}, 2), size(At{p}, 1), normAp, nnz(At{p}), nnz(At{p}) / numel(At{p}));
        if p == 1
            m = size(At{p}, 2);
        end
        n = n + size(At{p}, 1);
    ...
    normA = sqrt(normA);
    fprintf(fid, "\ttotal: %i x %i, norm = %.4e, nnz = %.4e, sparsity = %.4e\n", ...
        m, n, normA, MatCell.nnz(model.At), MatCell.sparsity(model.At));
