...
function [info, flag] = linsysfact(mat, info, mat_regu)
% comparing to linsysfact, linsysfact2 se the matrix as input 
...
if strcmp(info.solver_used, 'auto')
    ...
    if all(diag(mat) > 0) % PSD case
        if nnz(mat) / numel(mat) > 0.1 % dense case
...
elseif strcmp(info.solver_used, 'ldlsparse')  % ld for not neccessarily positive definite matrix
    [info, flag] = fact_ldlsparse(mat, info, mat_regu);
    if flag ~= 0
        warning('ldlsparse fail, switch to splu'); 
        info.solver_used = 'splu'; 
    end
elseif strcmp(info.solver_used, 'ldl')
    [info, flag] = fact_ldl(mat, info, mat_regu);
    if flag ~= 0
