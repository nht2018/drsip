function [std_model, transform] = standardize_forward(model)
    ...
    if is_identity(model.F(:, nonzero_cols)) && norm(model.g) < 1e-16
        ...
        std_model.b = b;
        std_model.lx = [model.lx(nonzero_cols); model.lx(zero_cols)];
        std_model.ux = [model.ux(nonzero_cols); model.ux(zero_cols)];
    else
        ...
        std_model.lx = [model.lx; -inf(size(model.F, 1), 1)];
        std_model.ux = [model.ux; inf(size(model.F, 1), 1)];
    ...
    while p <= length(std_model.K)
        ...
        if strcmp(cone.type, 'u')
            if all(std_model.lx(i: i + len - 1) == -inf) && all(model.ux(i: i + len - 1) == inf)
                % free variables
            else % bounded variables
                lx = std_model.lx(i: i + len - 1);
                ux = std_model.ux(i: i + len - 1);
            ...
            if all(std_model.lx(i: i + len - 1) == -inf) && all(std_model.ux(i: i + len - 1) == inf)
                % no other bounds
            elseif strcmp(std_model.K{p}.type, 'q') && all(std_model.lx(i: i + len - 1) == 0 | std_model.lx(i: i + len - 1) == -inf) && all(std_model.ux(i: i + len - 1) == inf) && std_model.lx(i) == 0 && all(std_model.lx(i + std_model.K{p}.size(1: end-1) ) == 0)
                % quadratic cone with only the head index have lower bound 0 in each sub-cone
            else
                std_model.K{p}.type = [std_model.K{p}.type,'+', 'b'];
                std_model.K{p}.params = [std_model.lx(i: i + len - 1), model.ux(i: i + len - 1)];
            end
        end
        p = p + 1;
        i = i + len;
    end
    rmfield(std_model, 'lx');
    rmfield(std_model, 'ux');
