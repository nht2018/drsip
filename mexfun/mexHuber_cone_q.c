 ...
 * LastEditTime: 2024-03-15 10:04:10
 * Description: the Huber function on the second order cone
 *      usage
 *      [X] = mexHuber_cone_q(Z, mu, cone_size)
 *      [X, eigval, Omega1, Omega2, U1, U2, shift] = mexHuber_cone_q(Z, mu, cone_size)
 *
 * Copyright (c) 2023, Hantao Nie, Peking University.
 */

// compute the Huber function on the second order cone also its Jacobian
...
//      Huber(x) = phi(lambda1, mu) * c1 + phi(lambda2, mu) * c2
// where phi(lambda, mu) = 0.5 * (lambda + sqrt(lambda * lambda + 4 * mu));
// the derivative of Huber(x) is given by
//      DHuber(x) = d * I + U * Omega * U'
...
// X: Huber function of Z onto the cone, i.e., X = Huber(Z) , size = [n, 1]
...
inline double Dphi(double z, double epsilon)
{
    if (z <= 0)
        return 0;
    else if (z <= epsilon)
...
#pragma omp parallel for private(j, i, z0, detz, norm2zbar, normzbar, phi1, phi2, temp) shared(cone_size, ind_head, k, Z, X, eigval, Omega1, Omega2, U1, U2, shift)
    ...
    for (j = 0; j < k; j++)
    {
        ...
        else // mu[j] > 0 perform Huber function
        {
            if (fabs(normzbar) < EPS)
            {
            ...
            // compute Jacobian of the Huber function
