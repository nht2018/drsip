 ...
 * LastEditTime: 2023-09-12 16:54:52
 * @Description: 
 * 
        ...
        if (nlhs == 4){
            if (fabs(rho - 1) < 1e-8){
                ...
                for (i=0; i<cone_size[j]; i++){
                    // DXcoeff[ind_head[j] + i] = - 2  * rho / ( (1 + rho)  / (1-rho) * X[ind_head[j]] * X[ind_head[j]] + (1 - rho) / ( 1 + rho) * norm2xbar  );
                    DXcoeff[ind_head[j] + i] = - 2  / ( detx / rho + 2  * ( X[ind_head[j]] * X[ind_head[j]] / (1-rho) + norm2xbar / (1+rho) ) );
                }
