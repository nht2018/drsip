...
function [stepsize, X_new, Z_new, model, algo] = linesearch(X, Z, dZ, mu, model, algo, params)
    ...
    algo.linesearch.sigma = 0.1;
    t0 = tic;
    b = model.b;

    X0 = projection(Z, model.K);
    merit0 = norm(algo.funcF(X0, Z)) ;
    

    compute_energy = @(X_, Z_) norm(algo.funcF(X_, Z_));

    merit_old = norm(algo.funcF(X, Z));
    ...
    while true
        ...
        merit_temp = compute_energy(X_new, Z_new); 
        if merit_temp - merit_old < -2 * algo.linesearch.sigma * stepsize * merit0 || stepsize <= 1e-5
            break;
        else
