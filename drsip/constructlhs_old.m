...
%  LastEditTime: 2024-03-01 14:43:46
%  
...
function [lhs, model, algo] = constructlhs(H_rd, model, algo, params)
    ...
    for p = 1:length(K)
        ...
        elseif strcmp(cone.type,'u')
            % if algo.iter == 1
            H_rd{p}.shift= 0; % actually H{p}.shift= inf, since we do no need to compute H for unbounded cone, we set it to 0 for convenience
            if ismember(algo.newton.system_opt, [1, 3])
    ...
    elseif algo.newton.system_opt == 2    
        ...
        [lhs.inv_mat22, lhs.mat] = schur_complement(lhs.mat2t, lhs.mat22_coeff, lhs.mat11, lhs.mat12, 'augmented') ;
