...
function [out, model, hist] = SN(model_in, params, init_point)
%% smoothing newton algorithm for solving conic optimization
...
% scale A
if params.scale_A
    [model, scale_A] = row_rescale(model);
    fprintf(fid, "[rescale A]\n");
    fprintf(fid, "average(row norm of A): %e\n", mean(scale_A));
else
    scale_A = ones(size(model.b));
end
algo.scale.scale_A = scale_A;
...
if params.init_strategy == 1 || params.precond_A || strcmp(params.method, 'fom') || params.boot_method == "fom"
    need_factAAt = true;
else
    need_factAAt = false;
end
...
if need_factAAt
    %% factorize AAt
    [model, algo] = factAAt(model, algo, params);
...
if params.precond_A
    ...
    precond = algo.AAt;
    assert(strcmp(precond.dense_column_strategy, 'none') && ~ algo.model.detect_very_sparse_rows);
    precond = rmfield(precond, 'time_fact');
    if strcmp(precond.solver, 'ldlchol')
        [precond.L, precond.D] = ldlsplit(precond.LD);
        precond.sqrtD = sqrt(precond.D);
    end
    model.b = fwsolve(precond, model.b);
...
if params.scale_bc_flag
    ...
    if model.n_box
        model.b_int(1: length(model.b)) = model.b;
        for p = 1: length(model.K)
            if strcmp(model.K{p}.type, 'b2l')
                model.c_int{p}(1: length(model.c{p})) = model.c{p};
            end
        end
    else
        model.b_int = model.b;
...
algo.Adagger = @(y) Atyfun(model.K, model.At_int, (invAAt(y, model, algo, params)) );    % if preconditioning is used, Adagger is equivalent to Atymap
...
res_rd_corr = 0;
res_nt_corr = 0;
algo.newton.n_iterative_refinement = 0;
...
algo.newton.ratio = 0;
algo.newton.kappa = 1e-3;
algo.newton.eta = 1;
algo.newton.tau_xs = 1;
algo.newton.tau_p = 1;
algo.newton.tau_d = 1;
algo.newton.r_xs = 1;
algo.newton.r_p = 1;
...
if ~isempty(init_point)
    ...
    if params.init_strategy == 1
        [mu, X, y, S, N, n_cones] = gen_init_point_nocedal(model, algo, params) ;
    elseif params.init_strategy == 2
        [mu, X, y, S, N, n_cones] = gen_init_point2(model, algo, params) ;
    elseif params.init_strategy == 3
        [mu, X, y, S, N, n_cones] = gen_init_point_sdpt3(model, algo, params) ;
    end
            
    if params.mu_init ~= -1
        mu = params.mu_init;
    elseif params.boot_method == "fom"
        mu = 0;
    end
...
algo.projection_func = @(Z) projection(Z, model.K);
algo.smooth_func = @(Z, mu) prox_neglog(Z, algo.AL_penalty * mu, model.K, params.det_reg, params.prox_neglog_formula);

% algo.smooth_func = @(Z, mu) smooth_poly_C2(Z, mu, model.K) ;
...
for iter = 0: params.max_iter
    ...
    %% decide method
    if strcmp(params.method, 'ipm')
        algo.newton.method = 'ipm' ;
        algo.newton.scaling = 'HKM';
    elseif strcmp(params.method, 'sn')
        print_head = 0;
        if params.boot_method == "ipm"
            if iter == 0 
                print_head = 1;
                algo.newton.method = 'ipm' ;
                algo.newton.scaling = 'HKM';
            elseif algo.newton.method == "ipm" && max(dinfint, pinfint) < 1e-3
                print_head = 1;
                algo.newton.method = 'sn' ;
                algo.newton.scaling = 'none';
            else % continue to use sn
        ...
        elseif params.boot_method == "fom"
            ...
            elseif algo.newton.method == "fom" && max(dinfint, pinfint) < 1e-3
            % elseif algo.newton.method == "fom" && max(dinfint, pinfint) < 1e-2
                % fprintf(fid, "Switch to DRS\n");
                print_head = 1;
                mu = max(inner_product(X, S ) / N, 1e-3);
                % mu = 0;
                step_p = 1;
                step_d = 1;
                algo.mu_decay = 0;
    ...
    if iter == 0 % initialize
        if strcmp(algo.newton.method, 'fom')
            stepsize = 1;
            n_fom_iter = 0;
        else % drs or ipm
            n_fom_iter = 0;
    ...
    elseif params.AL_penalty_update_flag && mod(iter, params.AL_penalty_update_iter) == 0 % update AL penalty and do one first-order step
        ...
        if algo.AL_penalty ~= AL_penalty_old
            ...
            %     [X, V] = algo.smooth_func(Z, algo.AL_penalty * mu);
    ...
    elseif strcmp(algo.newton.method, 'fom') % do one first-order step
        ...
        Aty = algo.Atymap_int(y);
        
        S = algo.projection_func(model.c - Aty - X / algo.AL_penalty);
        X = X + algo.AL_penalty * (Aty + S - model.c);
    elseif strcmp(algo.newton.method, 'sn') 
        if params.mu_decay ~= 0  % user specified
            algo.mu_decay = params.mu_decay;
        else % heuristic from sdpt3
            if params.predcorr
                algo.mu_decay = 0;
            else
                algo.mu_decay = 1-0.9*min(step_p,step_d);
                if (iter == 1); algo.mu_decay = 0.5; end;
                % if iter < 10; algo.mu_decay = min(0.5, algo.mu_decay); end;
            end
            % algo.mu_decay = 0;
        end
        
        % algo.mu_decay = 1;
        [dX_pred, dy_pred, dS_pred, res_rd_pred, res_nt_pred, model, algo] = sn_dir(X, y, S, mu, model, algo, params, 'pred');
        % % linesearch
        % if params.step_coeff == 0
        %     step_coeff = 0.9 + 0.09 * min(step_p, step_d);
        % else
        %     step_coeff = params.step_coeff;
        % end
        % [stepX] = longstep(X, dX_pred, model, algo, params);
        % step_p = min(1, step_coeff * stepX);
        % [stepS] = longstep(S, dS_pred, model, algo, params);
        % step_d = min(1, step_coeff * stepS);
        
        % X_pred = X + step_p * dX_pred;
        % y_pred = y + step_d * dy_pred;
        % S_pred = S + step_d * dS_pred;
        % % mu_pred = inner_product(X_pred, S_pred ) / N;
        % % mu_pred = mu * ( 1 - min(step_p, step_d) );
        
        [stepsize, X_pred, y_pred, S_pred, model, algo] = linesearch(X, y, S, dX_pred, dy_pred, dS_pred, mu, model, algo, params) ;
        step_p = stepsize;
        step_d = stepsize;
        % X_pred = algo.projection_func(X_pred);
        % S_pred = algo.projection_func(S_pred);
        mu_pred = mu * (1 - stepsize);
        %% corrector step
        if params.predcorr % && norm(dX_pred) / norm(X) < 0.1) && (norm(dS_pred) / norm(S) < 0.1 && norm(dX_pred - dS_pred) / norm(X - S) < 0.1)
            %% centering parameter heuristic from sdpt3
            step_pred = min(step_p, step_d);
            if (mu > 1e-6)
                if (step_pred < 1/sqrt(3))
                    expon = 1;
                else
                    expon = max(1, 3 * step_pred^2);
                end
            else
                expon = max(1, min(1, 3*step_pred^2));
            end
            if mu_pred < 0
                algo.mu_decay = 0.8;
            else
                % algo.mu_decay = min(1, (mu_pred / mu)^expon); % this is the original heuristic from sdpt3
                algo.mu_decay = min(0.99, (mu_pred / mu)^expon);
            end
            
            [dX_corr, dy_corr, dS_corr, res_rd_corr, res_nt_corr, model, algo] = sn_dir(X, y, S, mu, model, algo, params, 'corr', dX_pred, dS_pred);
            % dX = dX_corr;
            % dy = dy_corr;
            % dS = dS_corr;
            % %% linesearch
            % [stepX] = longstep(X, dX, model, algo, params);
            % [stepS] = longstep(S, dS, model, algo, params);
            % if params.step_coeff == 0
            %     step_coeff = 0.9 + 0.09 * min(step_p, step_d);
            % else
            %     step_coeff = params.step_coeff;
            ...
            % y = y + step_d * dy;
            % S = S + step_d * dS;
            % mu = inner_product(X, S) / N;

            [stepsize, X, y, S, model, algo] = linesearch(X, y, S, dX_corr, dy_corr, dS_corr, mu, model, algo, params) ;
            step_p = stepsize;
            step_d = stepsize;
            % X = algo.projection_func(X_pred);
            % S = algo.projection_func(S_pred);
            mu = mu * (1 - stepsize);
            newt_flag = 'cor';
        ...
        % assert(VarSDP.isinterior(model.K, X) )
        % assert(VarSDP.isinterior(model.K, S) )
    elseif  strcmp(algo.newton.method, 'ipm')
        if params.mu_decay ~= 0  % user specified
            algo.mu_decay = params.mu_decay;
        else % heuristic from sdpt3
            if params.predcorr
        ...
        % algo.mu_decay = 1;
        [dX_pred, dy_pred, dS_pred, res_rd_pred, res_nt_pred, model, algo] = ipm_dir(X, y, S, mu, model, algo, params, 'pred');
        % % linesearch
        % if params.step_coeff == 0
        %     step_coeff = 0.9 + 0.09 * min(step_p, step_d);
        % else
        ...
        [stepsize, ~, ~, ~, model, algo] = linesearch(X, y, S, dX_pred, dy_pred, dS_pred, mu, model, algo, params) ;
        step_p = stepsize;
        step_d = stepsize;
        % X_pred = algo.projection_func(X_pred);
        % S_pred = algo.projection_func(S_pred);
        ...
        if true
            ...
            [dX_corr, dy_corr, dS_corr, res_rd_corr, res_nt_corr, model, algo] = ipm_dir(X, y, S, mu_pred, model, algo, params, 'corr', dX_pred, dS_pred);
            dX = dX_corr;
            dy = dy_corr;
            dS = dS_corr;
            %% linesearch
            ...
            % step_p = stepsize;
            % step_d = stepsize;
            X = algo.projection_func(X);
            S = algo.projection_func(S);
            % mu = mu * (1 - stepsize);
            newt_flag = 'cor';
    ...
    if params.u2l && ismember(algo.newton.method, {'ipm', 'sn'})
        [X, y, S] = shift_cone_u2l(X, y, S, mu, step_d, u2lflag, model, algo, params);
    end
    ...
    pres = algo.presmap(X);
    pinfint = scale_b * norm(pres .* scale_A) / (1 + scale_b * norm(model.b .* scale_A));
    dres =  algo.dresmap(y, S);
    % dres = 1 / algo.AL_penalty * (X - X_old);
    dinfint = scale_c * norm(dres) / ( 1 + scale_c * norm(model.c));
    if model.n_box
        pobjint = scale_b * scale_c * inner_product(X, model.c_int) ;
        dobjint = scale_b * scale_c * model.b' * y;
        for p = 1 : length(model.K)
            cone = model.K{p};
            if strcmp(cone.type, 'b2l')
                dobjint = dobjint -  scale_b * scale_c * S{p}(cone.size / 2 + 1 : end)' * (cone.params(:, 2) - cone.params(:, 1));
                % dobjint = dobjint - scale_b * scale_c * (sum(S{p}(1: cone.size / 2) .* cone.params(:, 1)) - sum(S{p}(cone.size / 2 + 1: cone.size) .* cone.params(:, 2))) ;

            end
        end
    else
        pobjint = scale_b * scale_c * inner_product(X, model.c) ;
        dobjint = scale_b * scale_c * model.b' * y;
    end
    ...
    % % update kappa
    % comres = X - projection(X - algo.AL_penalty * S, model.K) ;
    % normF_new = sqrt( norm(pres) ^ 2 + norm(dres) ^ 2 + norm( comres) ^ 2  );
    % if params.newton_reg && iter > 0
    %     % ratio =  - <F, dZ> / norm(dZ) ^ 2
    %     normdZ2 = norm(dX)^2 + norm(dy)^2 + norm(dS)^2;
    %     if normF < 1e-3; normdZ2 = normF_new * normdZ2; end
    %     algo.newton.ratio = - ( sum(pres .*  dy) +  inner_product(dres, dX) + inner_product(comres, S)) / normdZ2;
    ...
    %     algo.newton.eta = eta_update(normF);
    % end
    % normF = normF_new;
    ...
    [X_org, y_org, S_org, dobjorg_box] = cone_b2l_recover(X, y, S, model);
    ...
    X_org = scale_b * X_org;
    y_org = scale_c * (bwsolve(model.precond, y_org) ./ scale_A);
    S_org = scale_c * S_org;
    ...
    if params.rescale_option
        ...
        S_org = S_org ./ algo.rescale.right_scale;
    end
    ...
    dinforg = norm(Atyfun(std_model.K, std_model.At, y_org) + S_org - std_model.c) / (1 + norm(std_model.c));
    ...
    if params.print_log
        ...
        if strcmp(algo.newton.method, 'fom')
            ...
            log_info = {'%6s', 'iter', '%6s', num2str(iter);
                ...
                '%6s', 'method', '%6s', algo.newton.method;
                '%7s', 'mu', '%7s', num2str(mu_avg, fm);
                '%7s', 'mu_dec', '%7s', num2str(algo.mu_decay, fm);
                '%8s', 'step_p', '%+8s', num2str(step_p, fm);
                '%8s', 'step_d', '%+8s', num2str(step_d, fm);
                '%8s', 'pobj', '%+8s', num2str(pobjorg, fm);
                '%8s', 'dobj', '%+8s', num2str(dobjorg, fm);
                };
        end
        ...
        if params.precond_A || params.rescale_option
            log_info = [log_info;
                {
                '%8s', 'gaporg', '%+8s', num2str(gaporg, fm);
                % '%8s', 'pobjorg', '%+8s', num2str(pobjorg, fm);
                % '%8s', 'dobjorg', '%+8s', num2str(dobjorg, fm);
                '%8s', 'pinforg', '%+8s', num2str(pinforg, fm);
                '%8s', 'dinforg', '%+8s', num2str(dinforg, fm);
        ...
        if params.print_log  == 2
            log_info = [log_info;
                ...
                '%8s', 'res_rd', '%+8s', num2str(res_rd_corr, fm);
                '%8s', 'res_corr', '%+8s', num2str(res_nt_corr, fm);
                % '%8s', 'q_denco', '%+8s', num2str(algo.newton.q_dencol);
                % '%8s', 'Ft(Aty+x', '%+8s', num2str(inner_product(F, algo.Atymap(y) + X / algo.AL_penalty), fm);
                % '%8s', 'ALpen', '%+8s', num2str(algo.AL_penalty, fm);
                '%8s', 'condH', '%+8s', num2str(algo.newton.condH, fm);
                }];
            if params.newton_reg
                log_info = [log_info;
                    {
                    '%8s', 'tau_xs', '%+8s', num2str(algo.newton.tau_xs, fm);
                    '%8s', 'tau_p', '%+8s', num2str(algo.newton.tau_p, fm);
                    '%8s', 'tau_d', '%+8s', num2str(algo.newton.tau_d, fm);
                    '%8s', 'ratio', '%+8s', num2str(algo.newton.ratio, fm);
                    '%8s', 'kappa', '%+8s', num2str(algo.newton.kappa, fm);
                    '%8s', 'eta', '%+8s', num2str(algo.newton.eta, fm);
                    }];
            end
            log_info = [log_info;
                {
            ...
            %     '%8s', 'dX_pred', '%+8s', num2str(norm(dX_pred), fm);
            %     '%8s', 'dS_pred', '%+8s', num2str(norm(dS_pred), fm);
                '%8s', 'r_xs', '%+8s', num2str(norm(algo.newton.r_xs), fm);
                '%8s', 'r_c', '%+8s', num2str(norm(algo.newton.r_c), fm);
                '%8s', 'r_p', '%+8s', num2str(norm(algo.newton.r_p), fm);
                '%8s', 'r_d', '%+8s', num2str(norm(algo.newton.r_d), fm);
                '%8s', 'res', '%+8s', num2str(sqrt( norm(algo.presmap(X))^2 + norm(algo.dresmap(y, S))^2 + norm(X - algo.projection_func(X - algo.AL_penalty * S))^2 ), fm);
...
out_summary = sprintf('%20s %20s\n%20s %20s\n%20s %20s\n%20s %20s\n%20s %20s\n%20s %20s\n%20s %20s\n', ...
    ...
    "Time (seconds)", num2str(toc(t0), '%.1f'),...
...
if ~isfield(params, 'print_log'); params.print_log = 2; end % 0: no log, 1: standard log, 2: verbose log
if ~isfield(params, 'print_params'); params.print_params = 1; end % 1: print, 0: no print
if ~isfield(params, 'tol'); params.tol = 1e-6; end
if ~isfield(params, 'stepsize_init'); params.stepsize_init = 0.9; end
if ~isfield(params, 'warning_on'); params.warning_on = 1; end
if ~isfield(params, 'log_path'); params.log_path = ''; end
if ~isfield(params, 'iterative_refinement_max_iter'); params.iterative_refinement_max_iter = 50; end
if ~isfield(params, 'iterative_refinement_tol'); params.iterative_refinement_tol = 1e-8; end
...
if ~isfield(params, 'scale_A'); params.scale_A = 0; end
...
if ~isfield(params, 'newton_reg'); params.newton_reg = 1; end
...
if ~isfield(params, 'first_order'); params.first_order_DRS = 0; end
...
if ~isfield(params, 'boot_method'); params.boot_method = "none"; end
